...
public class AddressBook extends JFrame implements ActionListener
{
    ...
    public static void main(String[] args)
    {
        ...
        contacts.setTitle("My address book: 3024246");
    ...
    private int addContact(String newName, String newAddress, String newMobile, String newEmail)
    {
        ...
        //If Statement checks if the database is full then if the database is full. the program will show a popup saying the program is full
        if(currentSize == databaseSize)
        {
            return -1;
        }
        //Else statement means that more entries can be added to the database if there is space
        else
        {
    ...
    private void deleteContact(int index)
    {
        ...
        //The code below blanks out the data within the arrays on the currently selected contact

        //Check if the arrays are empty
        if(currentSize>0)
        {
            //Blank out the contacts in the selected position
            name[currentContact] = "";
            address[currentContact] = "";
            mobile[currentContact] = "";
            email[currentContact] = "";
            //Make sure the currentSize variable drops by 1 in order to ensure free space in the database
            currentSize = currentSize - 1;
            //DoWhile Loop setup created to move the contacts above the deleted contact one position down
            do
            {
                //This is done by moving the index position down by 1
                name[index] = name[index + 1];
                address[index] = address[index + 1];
                mobile[index] = mobile[index + 1];
                email[index] = email[index + 1];
                index++;
                //Index is incremented in order to make sure that there isn't simply a hidden position,
                // but instead that the position lowers
            }
            while(index < currentSize);
            //While statement above just checks whether the value in index is below the currentSize variable in order
            //to ensure that the index positions don't get pushed out of the boundaries.
            //There is very likely a better way to explain why it's done this way, but the real reason why I did it this
            //way was because the program froze and refused to close when this 'while' statement was placed in the position
            //of 'do'. It's probably something to do with hardware limitations on my laptop, but, I've included this in
            //the video for evidence
    ...
    private void clearContacts()
    {
        ...
        //For loop is set up to iterate through the values in the array with the parameters being the length of the name array
        //This is not done with the "currentSize" variable because that doesn't actually delete entries, it just makes them
        //Invisible. Trust me, I tried it!
        for (int i = 0; i < name.length; i++) {
            //The values in each of the arrays at the index point "i" are set to be blanked, hence deleting the entry and freeing up space
            name[i] = "";
            address[i] = "";
            mobile[i] = "";
            email[i] = "";
            //The value of variable "currentSize" is dropped down by 1, because otherwise the program thinks that there
            // is still data in the array. This is only done if the currentSize is above 0 as it will force the variable
            // into negative values otherwise.
            if (currentSize > 0)
            {
                currentSize = currentSize - 1;
            }
    ...
    {
        // TO BE DONE: Implement this method body - see comments above
        //For Loop set up to iterate through the array with currentSize set as the max
        for (int i = 0; i < currentSize; i++)
        {
            //If statement checks whether the string searchName exactly matches the entry at position i in the array
            //If it is not found to be an exact match then there will be a second iteration to check it against the rest
            //of the names
            if (searchName.equals(name[i])) //.equals used on the presumption that as the quicksort method would require .compareTo, the other string operators should be allowed
            {
                return i; //Returns i where found, moving the current position of the database to where the loop found a match
            }
    ...
    private int findPartial(String searchText)
    {
        ...
        //For loop setup again to iterate through the array name[] and find where searchText contains characters from name[i] incremented by 1 each time no match is found
        for (int i = 0; i < currentSize; i++)
        {
            //The elements in name, sorted or not, are all temporarily set to lower case in order to produce a match.
            if(name[i].toLowerCase().contains(searchText.toLowerCase())) //String operator .contains used to check if the searchText variable produces matches within the name array
                //.toLowerCase() used in above line on the array name[] and the variable searchText. Doing this outside
                // the for loop will throw out an error presumably because the .toLowerCase function only works within
                // the line the code is being executed in, unless a new variable is declared
            {
                return i; //Moves database to position i where a partial match (or full if you just straight up typed the whole name in) is found within array name[]
            }
        }
        return -1;                          // Return where found or -1
    } // End of findPartial
    ...
    private void sortAtoZ()
    {
        ...
        //Initialise i index point 0, Loop will check if i is below the current size of database
        for(int i  = 0 ; i < currentSize ; i++)
        {
            //Initialise j at index point 0, Loop will check if j is below the current size of database
            for(int j = 0 ; j < currentSize ; j++)
            {
                //This is effectively comparing the name string at one point in the array to the names at other points
                //in the array.
                if(name[i].compareTo(name[j]) < 0)
                {
                    //If the name is found to be lower in the alphabet than the names above it, it will be placed in the
                    // index position below all entries which return a negative value
                    String tempName = name[i];
                    name[i] = name[j];
                    name[j] = tempName;

                    String tempAddress = address[i]; //Other array elements are shifted to match the name they correspond to
                    address[i] = address[j];
                    ...
                    String tempMobile = mobile[i];
                    mobile[i] = mobile[j];
                    mobile[j] = tempMobile;

                    //Loop will repeat until the array is fully completed, swapping elements in the case that they are
                    // lower in the alphabet, or doing nothing if the order is correct.
                }
            }
        }
    ...
    private void sortZtoA()
    {
        ...
        //Initialise i at index point 0, Loop will check if i is below the current size of database
        for(int i  = 0 ; i < currentSize ; i++)
        {
            //Initialise j at index point 0, Loop will check if j is below the current size of database
            for(int j = 0 ; j < currentSize ; j++)
            {
                //This is effectively comparing the name string at one point in the array to the names at other points
                //in the array.
                if(name[i].compareTo(name[j]) > 0) //Simple change in a single character from the sortAtoZ method:
                    // changing < to > gets the method to sort the arrays in reverse order
                {
                    //If the name is found to be higher in the alphabet than the names above it, it will be placed in the
                    // index position below all entries which return a positive value. Hence sorting the names from Z to A
                    String tempName = name[i];
                    name[i] = name[j];
                    name[j] = tempName;
                    ...
                    //Loop will repeat until the array is fully completed, swapping elements in the case that they are
                    // higher in the alphabet, or doing nothing if the order is correct.
                }
            }
        }
        //This course is shit, please someone help me, Also Hi Creative Writing Folk
    }
